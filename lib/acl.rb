require 'acl/interface'
require 'role/interface'
require 'resource/interface'
Dir[File.dirname(__FILE__) + '/*/*.rb'].each do |f|
  require f
end

module Hubracl
  class Acl < Acl::Interface
    ...
    def add_role(role, parent = nil)
      if (role.is_a?(String) || role.is_a?(Symbol)) == true
        role = Acl::Role.new(role)
      end
      unless role.is_a? Acl::Role::Interface
        raise Acl::Exception::InvalidArgumentException("role must be an instance of class Acl::Role::Interface (children are fine).")
      end
      get_role_registry().add(role, parents)
    ...
    def add_resource(resource, parent = nil)
      if (resource.is_a?(String) || resource.is_a?(Symbol)) == true
        resource = Acl::Resource.new(resource)
      end
      unless resource.is_a? Acl::Resource::Interface
        raise Acl::Exception::InvalidArgumentException("resource must be an instance of class Acl::Resource::Interface (children are fine).")
      end

      resource_id = resource.get_resource_id()

      if has_resource?(resource_id)
        raise Acl::Exception::InvalidArgumentException("Resource ID #{resource_id} already exists in the ACL")
      end
      ...
      if parent != nil
        begin
          if parent.is_a? Acl::Resource::Interface
            resource_parent_id = parent.get_resource_id()
          else
            resource_parent_id = parent
          end
        rescue
          raise Acl::Exception::InvalidArgumentException("Parent Resource ID #{resource_parent_id} does not exist.")
        end
    ...
    def get_resource(resource)
      if resource.is_a? Acl::Resource::Interface
        resource_id = resource.get_resource_id()
      else
      ...
      unless has_resource?(resource)
        raise Acl::Exception::InvalidArgumentException("Resource #{resource_id} not found.")
      end
    ...
    def inherits_resource(resource, inherit, only_parent = false)
      ...
      rescue Acl::Exception::Interface => e
        raise Acl::Exception::InvalidArgumentException(e.message)
      end
    ...
    def set_rule(operation, type, roles = nil, resources = nil, privileges = nil, assert = nil)
      ...
      if @@TYPE_ALLOW != type && @@TYPE_DENY != type
        raise Acl::Exception::InvalidArgumentException.new("Unsupported rule type; must be either #{@@TYPE_ALLOW} or #{@@TYPE_DENY}")
      end
      ...
      case(operation)
        ...
        when @@OP_REMOVE
          ...
          raise Acl::Exception::InvalidArgumentException("Unsupported operation; must be either #{@@OP_ADD} or #{@@OP_REMOVE}.")
      end
    ...
    def isAllowed(role = nil, resources = nil, privileges = nil)
      @is_allowed_role = nil
      ...
      if role == nil
        ...
        if !@is_allowed_role.is_a? Acl::Role::Interface
          @is_allowed_role = role
        end
      end
      ...
      if resource == nil
        ...
        if !@is_allowed_resource.is_a? Acl::Resource::Interface
          @is_allowed_resource = resource
        end
      end
        ...
        begin
          ...
          if (rules = get_rules(resource, nil)) != nil
            rules[:by_privilege_id].each { |privilege, rule|
              if @@TYPE_DENY == (rule_typ_on_privilege = get_rule_type(resource, nil, privilege))
                return false
              end
            }
    ...
    def get_role_registry
      @role_registy ||= Acl::Role::Registry.new
    end
    ...
    def get_rule_type(resource = nil, role = nil, privilege = nil)
      ...
      if rule[:assert]
        ...
        assertion_value = assertion.assert(
          self,
          @is_allowed_role.is_a?(Acl::Role::Interface) ? @is_allowed_role : role,
          @is_allowed_resource.is_a?(Acl::Resource::Interface) ? @is_allowed_resource : resource,
